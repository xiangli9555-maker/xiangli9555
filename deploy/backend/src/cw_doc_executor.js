...
// lines: [{ role_cn, cn_text, en_text, situation?, trigger?, remark? }]
// 从第 2 行（row=1）起写；只写列 1(游戏角色名)/2(台词-中)/3(台词-英)/4(情绪)/5(触发条件)/7(备注)。
// 不清空原有行——只是"追加/覆盖"从第2行起 lines.length 行。用于下午演示 demo，事后配套 cleanup 清除。
