...
for (const [index, page] of demandPages.entries()) {
  ...
  test(`需求汇总页 ${index + 1} 使用真实数据时间并展示快照时间`, () => {
    ...
    assert.equal(/Tapd 数据上次拉取/.test(page), true, '缺少 Tapd 上次拉取时间文案');
  ...
  test(`需求汇总页 ${index + 1} 「从 TAPD 刷新」为实时拉取（放开 dfai_live 源 + 去「导入」文案）`, () => {
    assert.equal(/从 TAPD 刷新导入/.test(page), false, '按钮文案仍含「导入」，未体现实时语义');
    assert.equal(/从 TAPD 刷新/.test(page), true, '按钮文案未更新为「从 TAPD 刷新」');
...
test('1470×956 需求汇总保留 15 列并禁止宽表横向滚动', () => {
  demandPages.forEach((page, index) => {
    ...
    assert.match(page, /table\.demand-table col:nth-child\(n\+8\):nth-child\(-n\+13\)\{width:4\.1%!important\}/, '六个声优预估大类 col 必须按百分比收缩');
    assert.match(page, /table\.demand-table col:nth-child\(n\+14\)\{width:8\.8%!important\}/, 'Max 中英两列必须保留');
