...
// ---------- 3. 声优视图骨架（2026-09-07：回退到与需求视图同构的 6 板标签云） ----------
test('声优视图使用 6 板标签云并保留零数据骨架', () => {
  assert.match(SRC, /function renderActorSixBoard\(/, '应存在共用 6 板入口');
  assert.match(SRC, /function renderActorRoleTable\(\)\{\s*return\s*renderActorSixBoard\('role'\);?\s*\}/, '声优视图应恢复为 role 六板薄包装');
  assert.match(SRC, /lbe-skel|lb-skeleton|待录入声优预估/, '应给出与需求视图一致的骨架语义');
  assert.doesNotMatch(SRC, /role-estimate-card/, '不得残留独立角色卡片视图');
});
