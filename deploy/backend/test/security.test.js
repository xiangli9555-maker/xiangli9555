...
test('valid token resolves server-side subject and role', () => {
  const { req, passed } = authenticate(TOKENS.editor);
  assert.equal(passed, true);
  assert.deepEqual(req.auth, { subject: 'editor-user', role: 'editor' });
});

test('invalid token is rejected', () => {
  const { res, passed } = authenticate('not-valid');
  assert.equal(passed, false);
  assert.equal(res.statusCode, 401);
  assert.equal(res.body.error, 'unauthorized');
});

test('viewer can read but cannot write or delete', () => {
  const { req } = authenticate(TOKENS.viewer);
...
test('admin can delete', () => {
  const { req } = authenticate(TOKENS.admin);
  req.method = 'DELETE';
  let passed = false;
  methodRbac(req, responseMock(), () => { passed = true; });
  assert.equal(passed, true);
